...
namespace SpectrumChart
{
    ...
    class ACT12816Vibrate : ACT12x
    {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (success)
                    {
                        ...
                        //        byte[] result = string result = JsonConvert.SerializeObject(awObject);
        ...
        {
            int numberOfPointsInChart = WindowSize;
            if (isUpdateChart)
            {
        ...
        void ProcessFrame(float[,] channels, string stamp)
        {
            ...
            for (int i = 0; i < signal.Channels; i++)
            {
                ...
                //normalize amplitude
                for (int n = 1; n < power[i].Length; n++)
                {
